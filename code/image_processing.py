...
def process_data(shift=1):
    ...
    labels = data["target"]
    features = data.drop(["target", "future_price", "time"], axis=1)
...
def optimise(labels, features, n_iter=30, init_points=10):
    opt = BayesianOptimization(
        f=lambda n_estimators, max_depth: rf_eval(
            labels, features, n_estimators, max_depth
        ),
...
def plot_perm_importance(results):
    os.makedirs("resources/plots", exist_ok=True)
