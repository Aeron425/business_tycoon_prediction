...
def xgb_eval(labels, features, n_estimators, max_depth, min_child_weight, scale_pos_weight):
    model = XGBClassifier(
...
def optimise(labels, features, n_iter=30, init_points=10):
    opt = BayesianOptimization(
        f=lambda n_estimators, max_depth, min_child_weight, scale_pos_weight: xgb_eval(
            labels, features, n_estimators, max_depth, min_child_weight, scale_pos_weight
        ),
        pbounds={
            "n_estimators": (50, 500),
            "max_depth": (3, 15),
...
def evaluate(labels, features, best_params, shift):
    ...
    with open(f"resources/models/xgb_shift_{shift}.pkl", "wb") as f:
        pickle.dump(model, f)
...
def plot_results(results):
    ...
    plt.tight_layout()
    plt.savefig("resources/plots/xgb_shift_comparison.png", dpi=120)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig("resources/plots/xgb_perm_importance.png", dpi=120)
    plt.close()
